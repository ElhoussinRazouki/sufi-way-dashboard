...
export default function PdfUploaderField({
  name,
  className,
  dropZoneClassName,
  label,
  ...
  const handleOnUploadFinish = ({ files }: onUploadProps) => {
    // Enable Loading
    APIs.common
      .uploadAttachment(files[0], name, 'pdf', handleUploadProgress)
      .then((attachment) => {
        if (attachment) {
